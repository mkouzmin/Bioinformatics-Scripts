...
class CommandLine():
    def __init__(self, inOpts=None):
        ...
        self.parser = argparse.ArgumentParser(
            description='Program parses a multiple alignment into transmission and emmision probabilities',
...
class Alignment:
    def __init__(self, alignmatrix, inprob):#takes matrix of allignments
        ...
        alilen = alidim[1]  # length of individual allignment
        alicount = alidim[0]  # number of given allignments
        ...
        for col in self.alignmatrix.T:
            ...
            for i in numpy.nditer(col):
                if i == "-":
                    inscount += 1
            if (inscount / alicount) > self.threshold:
                self.insertlist.append(colnum)
            colnum += 1
        self.vallen = alilen - len(self.insertlist)#get number of valued columns in alignment
...
def main(myCommandLine=None):
    ...
    try:
        ...
        inprob = read.readline()[:-1]#get threshold
        read.readline()
