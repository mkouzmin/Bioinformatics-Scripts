            ...
            for k,v in self.corr_T_Dict.items():
                if k[0] == "S":
                    ...
                    if i == 1:#if in left column
                        self.Vmatrix[v][i] = self.Vmatrix[v - 1][i - 1] + numpy.log( self.get_T_Value(MMatrices, v - 1, v) \
                        ...
                        self.Vmatrix[v][i] = self.Vmatrix[v][i-1] + numpy.log( self.get_T_Value(MMatrices, v, v)\
                                             * self.get_E_Value(MMatrices,v,self.path[i-1]))
                        self.Pmatrix[v][i] = v
                        ...
                        for x in [v,v-1,v-2]:
                            num = self.Vmatrix[x][i-1] + numpy.log( self.get_T_Value(MMatrices, x, v)\
                                                 * self.get_E_Value(MMatrices,v,self.path[i-1]))
                    ...
                    if i == 1:#if in left column
                        self.Vmatrix[v][i] = self.Vmatrix[v - 2][i - 1] + numpy.log( self.get_T_Value(MMatrices, v - 2, v) \
                                             * self.get_E_Value(MMatrices, v, self.path[i - 1]))
                    ...
                    if i == 0:# if in left column
                        self.Vmatrix[v][i] = self.Vmatrix[v - 3][i] + numpy.log( self.get_T_Value(MMatrices, v - 3, v))
                        self.Pmatrix[v][i] = v-3
                        #self.Dmatrix[v][i] = 0 - numpy.log(self.get_T_Value(MMatrices, v - 3, v))
                    elif k[1:] == "1":# if in top row
                        self.Vmatrix[v][i] = self.Vmatrix[v - 2][i] + numpy.log( self.get_T_Value(MMatrices, v - 2, v))
                        self.Pmatrix[v][i] = v-2
                        #self.Dmatrix[v][i] = 0 - numpy.log(self.get_T_Value(MMatrices, v - 2, v))
                    else: # general case
                        for x in [v-2,v-3,v-4]:
                            num = self.Vmatrix[x][i] + numpy.log(self.get_T_Value(MMatrices, x, v))
                            if self.Vmatrix[v][i] < num:
                                self.Vmatrix[v][i] = num
                                self.Pmatrix[v][i] = x
                                #self.Dmatrix[v][i] = 0 - numpy.log(self.get_T_Value(MMatrices, x, v))
